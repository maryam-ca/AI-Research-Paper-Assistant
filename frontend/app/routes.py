...
from app.metadata_fetcher import (
...
def _serialize_paper(p: Paper) -> dict:
    ...
    return {
        ...
        "started_reading_at": (
            p.started_reading_at.isoformat()
            if p.started_reading_at
            else None
        ),
        "last_read_at": (
        ...
        "completed_reading_at": (
            p.completed_reading_at.isoformat()
            if p.completed_reading_at
            else None
        ),
...
def get_elements(
    ...
    e = (
        ...
        .filter(
            KeyElements.paper_id == pid(paper_id)
        )
        .order_by(
            KeyElements.extracted_at.desc()
        )
        .first()
    )
...
def get_questions(
    ...
    rows = (
        db.query(QAHistory)
        .filter(
            QAHistory.paper_id == pid(paper_id)
        )
        .order_by(
            QAHistory.created_at.asc()
        )
        .all()
    )
...
def related_papers(
    ...
    sub = (
        ...
        .filter(
            CollectionPaper.paper_id == target
        )
        .subquery()
    )
    ...
    related = (
        ...
        .filter(
            CollectionPaper.collection_id.in_(sub)
        )
        .filter(
            Paper.id != target
        )
...
def export_markdown(
    ...
    if format == "notion":
        notion = {
            "parent": {
                "type": "database_id",
                "database_id": "REPLACE_WITH_NOTION_DB",
            },
            "properties": {
                "Title": {
                    "title": [
...
def get_highlights(
    ...
    rows = (
        db.query(Highlight)
        .filter(
            Highlight.paper_id == pid(paper_id)
        )
        .order_by(
...
def list_rq(
    ...
    rows = (
        ...
        .filter(
            ResearchQuestion.collection_id
            == cid
        )
        .order_by(
...
def add_to_collection(
    ...
    exists = (
        ...
        .filter(
            CollectionPaper.collection_id == cid,
            CollectionPaper.paper_id == pid_val,
        )
        .first()
    )
    ...
    if out_format == "bibtex":
        bib = "\n".join(
            (
                ...
                f"  title = {{{p.title}}},\n"
                f"  author = "
                f"{{{' and '.join(p.authors or [])}}},\n"
                f"  note = "
                f"{{ScholarFlow reading list: "
                f"{c.name}}}\n"
                f"}}"
            )
            for idx, p in enumerate(
